...
func (s *SalStore) GetAllBets(ctx context.Context, req GetAllBetsReq) (GetAllBetsResp, error) {
	...
	for rows.Next() {
		...
		respMap.AppendTo("state", &resp.State)
		...
		respMap.AppendTo("updated_at", &resp.UpdatedAt)
...
func (s *SalStore) GetBet(ctx context.Context, req GetBetReq) (GetBetResp, error) {
	...
	stmt, err := s.ctrl.PrepareStmt(ctx, s.parent, s.handler, pgQuery)
	if err != nil {
		return GetBetResp{}, errors.WithStack(err)
	}
	...
	rows, err := stmt.QueryContext(ctx, args...)
	if err != nil {
		return GetBetResp{}, errors.Wrap(err, "failed to execute Query")
	}
	defer rows.Close()

	cols, err := rows.Columns()
	if err != nil {
		return GetBetResp{}, errors.Wrap(err, "failed to fetch columns")
	}

	if !rows.Next() {
		if err = rows.Err(); err != nil {
			return GetBetResp{}, errors.Wrap(err, "rows error")
		}
		return GetBetResp{}, sql.ErrNoRows
	}
	...
	if err = rows.Scan(dest...); err != nil {
		return GetBetResp{}, errors.Wrap(err, "failed to scan row")
	}

	if err = rows.Err(); err != nil {
		return GetBetResp{}, errors.Wrap(err, "something failed during iteration")
	}
...
func (s *SalStore) PrepareExpressionFirst(ctx context.Context, req *PrepareExpressionFirstReq) error {
	var (
		err      error
		rawQuery = req.Query()
		reqMap   = make(sal.RowMap)
	...
	ctx = context.WithValue(ctx, sal.ContextKeyTxOpened, s.txOpened)
	ctx = context.WithValue(ctx, sal.ContextKeyOperationType, "Exec")
	ctx = context.WithValue(ctx, sal.ContextKeyMethodName, "PrepareExpressionFirst")
...
func (s *SalStore) PrepareExpressionSecond(ctx context.Context, req *PrepareExpressionSecondReq) error {
	var (
		err      error
		rawQuery = req.Query()
		reqMap   = make(sal.RowMap)
	...
	ctx = context.WithValue(ctx, sal.ContextKeyTxOpened, s.txOpened)
	ctx = context.WithValue(ctx, sal.ContextKeyOperationType, "Exec")
	ctx = context.WithValue(ctx, sal.ContextKeyMethodName, "PrepareExpressionSecond")
...
func (s *SalStore) UpdateBet(ctx context.Context, req UpdateBetReq) (*UpdateBetResp, error) {
	...
	reqMap.AppendTo("id", &req.ID)
	reqMap.AppendTo("state", &req.State)
